...
def generate_file_name(current_month, current_year):
    filename = (str(current_year) + '_' + str(current_month) + '_AggregatedGenerationPerType.csv')
...
def download_file(filename, dl_path):
    with pysftp.Connection(entsoe_ftp['host'], username=entsoe_ftp['user'], password=entsoe_ftp['password'],
        ...
        with sftp.cd(entsoe_ftp['folder'] + 'AggregatedGenerationPerType/'):
...
def clean_database(current_month, current_year):
    with dwh_conn.cursor() as cur:
        cur.execute("DELETE FROM stg_entsoe.agppt WHERE year = " + str(current_year) + " AND month = " + str(current_month) + ";")
        dwh_conn.commit()


def load_data_to_db(filename):
    with dwh_conn.cursor() as cur:
        cur.execute("COPY stg_entsoe.agppt FROM '" + dl_path + '/' + filename + "' WITH (DELIMITER E',', FORMAT CSV);")
        dwh_conn.commit()
